...
public final class RenderedTextBridge {
    ...
    public static Text translateEntityName(Entity entity, Text text) {
        ...
        if (shouldHideEntityName(entity)) {
            // 1.16.5 名牌渲染不判空，此处必须原样返回
            return text;
        }
    ...
    /** 第一人称下自己的名牌应整段隐藏 */
    public static boolean shouldHideEntityName(Entity entity) {
        if (entity == null) {
            return false;
        }
    ...
    public static OrderedText translate(
        ...
        MutableText replacement = new LiteralText(translated).setStyle(translatedStyle(firstStyle));
    ...
    public static StringVisitable translate(StringVisitable text) {
        ...
        Text base = new LiteralText(translated).setStyle(translatedStyle(Style.EMPTY));
    ...
    private static Text rebuildStyledText(
        ...
        if (base == null) {
            base = new LiteralText(translated).setStyle(translatedStyle(source.getStyle()));
        }
