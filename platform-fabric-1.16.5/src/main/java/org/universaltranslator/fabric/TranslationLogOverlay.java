...
public final class TranslationLogOverlay {
    ...
    public static void render(MatrixStack matrices, float tickDelta) {
        ...
        String text = config.displayMode == org.universaltranslator.core.TranslationDisplayMode.ORIGINAL_AND_TRANSLATED
                ? entry.original + "\n" + entry.translated
                : entry.translated;
        List<OrderedText> lines = textRenderer.wrapLines(new LiteralText(text), scaledWidth - 12);
        int visibleLines = Math.min(lines.size(), 8);
        int scaledHeight = 8 + visibleLines * (textRenderer.fontHeight + 2);
        matrices.push();
        matrices.translate(x, y, 0.0F);
        matrices.scale(scale, scale, 1.0F);
        DrawableHelper.fill(matrices, 0, 0, scaledWidth, scaledHeight, 0xAA000000);
        DrawableHelper.fill(matrices, 0, scaledHeight - 1, scaledWidth, scaledHeight, 0x66FFFFFF);
        int lineY = 5;
        for (int index = 0; index < visibleLines; index++) {
            textRenderer.drawWithShadow(matrices, lines.get(index), 6.0F, lineY, 0xFFFFFFFF);
