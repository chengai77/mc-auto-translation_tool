...
final class BookTextStyler {
    ...
    static Text rebuild(
        ...
        if (styled.allVisibleTextUses(sourceRuns.get(0).style())) {
            return new LiteralText(translated).setStyle(displayStyle(sourceRuns.get(0).style()));
        }
        ...
        if (targets.isEmpty()) {
            return new LiteralText(translated).setStyle(plainStyle);
        }
    ...
    private static boolean isDecorated(Style style) {
        ...
        return style.getColor() != null
                ...
                || style.isObfuscated()
                || (style.getFont() != null && !Style.DEFAULT_FONT_ID.equals(style.getFont()));
    }
