...
final class TranslationLogScreen extends Screen implements LocalTranslationScreen {
    ...
    TranslationLogScreen(Screen parent) {
        super(new TranslatableText("screen.universal_translator.translation_log"));
        this.parent = parent;
    }
    ...
    private void rebuildWidgets() {
        ...
        addDrawableChild(TranslatorUiText.button(12, bottom, 90, BUTTON_HEIGHT,
                new TranslatableText("gui.back"), button -> onClose()));
        addDrawableChild(TranslatorUiText.button(108, bottom, 100, BUTTON_HEIGHT,
                new TranslatableText("screen.universal_translator.log.clear"), button -> {
            FabricTranslationRuntime.clearTranslationHistory();
            scroll = 0;
            draggingScrollbar = false;
            rebuildWidgets();
        }));
        addDrawableChild(TranslatorUiText.button(216, bottom, 118, BUTTON_HEIGHT,
                new TranslatableText("screen.universal_translator.log.source_settings"), button ->
                        MinecraftClient.getInstance().setScreen(new TranslationLogSourceScreen(this))));
        pinnedButton = addDrawableChild(TranslatorUiText.button(width - 102, bottom, 90, BUTTON_HEIGHT, pinnedText(), button -> {
        ...
        int y = bottom - 54;
        addDrawableChild(TranslatorUiText.button(12, y, CONTROL_WIDTH, BUTTON_HEIGHT,
                new TranslatableText("screen.universal_translator.log.top_left"), button -> {
        ...
        addDrawableChild(TranslatorUiText.button(92, y, CONTROL_WIDTH, BUTTON_HEIGHT,
                new TranslatableText("screen.universal_translator.log.top_right"), button -> {
            FabricConfig current = FabricTranslationRuntime.currentConfig();
            if (current != null) {
        ...
        addStepper(172, y + 26, "Y", config.pinnedLogY, -4, 4, 0, 500, (current, value) ->
                savePinned(true, current.pinnedLogPreset, current.pinnedLogX, value,
                        current.pinnedLogWidth, current.pinnedLogScale));
        addStepper(332, y + 26, new TranslatableText("screen.universal_translator.log.width").getString(),
        ...
        addStepper(492, y + 26, new TranslatableText("screen.universal_translator.log.scale").getString(),
    ...
    private void addStepper(int x, int y, String label, int value, int decrement, int increment,
        ...
        addDrawableChild(TranslatorUiText.button(clampedX, y, 24, BUTTON_HEIGHT,
                new LiteralText("-"), button -> step(value + decrement, minimum, maximum, action)));
        addDrawableChild(TranslatorUiText.button(clampedX + 26, y, 82, BUTTON_HEIGHT,
                new LiteralText(label + " " + value), button -> { }));
        addDrawableChild(TranslatorUiText.button(clampedX + 110, y, 24, BUTTON_HEIGHT,
                new LiteralText("+"), button -> step(value + increment, minimum, maximum, action)));
    }
    ...
    private Text pinnedText() {
        FabricConfig config = FabricTranslationRuntime.currentConfig();
        return new TranslatableText(config != null && config.pinnedLogEnabled
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        DrawableHelper.drawCenteredText(matrices, textRenderer, title, width / 2, 12, 0xFFFFFFFF);
        renderEntries(matrices);
    }

    private void renderEntries(MatrixStack matrices) {
        ...
        int y = top - scroll;
        if (entries.isEmpty()) {
            DrawableHelper.drawCenteredText(matrices, textRenderer,
                    new TranslatableText("screen.universal_translator.log.empty"),
            ...
            if (y + entryHeight >= top && y <= bottom) {
                ...
                for (OrderedText line : lines) {
                    if (lineY >= top && lineY <= bottom - textRenderer.fontHeight) {
                        textRenderer.drawWithShadow(matrices, line, 24, lineY, 0xFFFFFFFF);
                    }
                    lineY += textRenderer.fontHeight + 2;
                }
                int separatorY = y + entryHeight - 7;
                DrawableHelper.fill(matrices, 24, separatorY, width - 32, separatorY + 1, 0x44FFFFFF);
            }
            y += entryHeight;
        }
        renderScrollbar(matrices, top, bottom, maxScroll);
    }

    private List<OrderedText> wrappedLines(TranslationLog.Entry entry, int contentWidth) {
        return textRenderer.wrapLines(new LiteralText(entryText(entry)), contentWidth);
    }
    ...
    private void renderScrollbar(MatrixStack matrices, int top, int bottom, int maxScroll) {
        ScrollbarMetrics metrics = scrollbarMetrics(top, bottom, maxScroll);
        if (metrics == null) {
            return;
        }
        DrawableHelper.fill(matrices, metrics.barX, top, metrics.barX + 5, bottom, 0x44000000);
        DrawableHelper.fill(matrices, metrics.barX, metrics.thumbTop, metrics.barX + 5,
