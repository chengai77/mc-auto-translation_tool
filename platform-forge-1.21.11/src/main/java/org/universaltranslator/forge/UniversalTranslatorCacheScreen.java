...
final class UniversalTranslatorCacheScreen extends Screen {
        ...
        if (client != null) {
            client.setScreen(new ConfirmScreen(confirmed -> {
                ...
                if (confirmed) {
                    ...
                    runAsync(() -> {
                        ForgeTranslationRuntime.clearCacheFile();
                        return 0;
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (!status.isEmpty()) {
            context.drawCenteredTextWithShadow(textRenderer, Text.literal(status), width / 2, height - 48, 0xD0D0D0);
        }
