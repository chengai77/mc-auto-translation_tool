...
public final class HologramTextDisplayGroups {
    ...
    public static Result translate(
        ...
        Component translated = row.translated();
        if (translated == null) {
            return Result.original(source);
        }
    ...
    public static final class Result {
        ...
        private Result(Component text, float horizontalOffset, boolean hidden, boolean centered) {
            this.text = text;
        ...
        static Result original(Component text) {
            return new Result(text, 0.0F, false, false);
        }

        static Result hiddenResult() {
            return new Result(Component.empty(), 0.0F, true, false);
        }

        static Result grouped(Component text, float horizontalOffset) {
    ...
    private static final class FragmentState {
        ...
        private FragmentState(SpatialHologramLayout.Fragment layout, Component text) {
            this.layout = layout;
            this.text = text;
        }
    ...
    private static final class PreparedRow {
        ...
        private volatile Component translated;

        private PreparedRow(int hostId, float centerX, Component source) {
            this.hostId = hostId;
        ...
        synchronized Component translated() {
            if (translated != null) {
                return translated;
            }
            Component candidate = RenderedTextBridge.translatePlainHologramText(source);
