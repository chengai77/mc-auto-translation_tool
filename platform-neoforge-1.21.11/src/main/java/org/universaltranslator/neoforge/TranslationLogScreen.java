...
final class TranslationLogScreen extends Screen {
    ...
    private void rebuildWidgets() {
        ...
        addDrawableChild(ButtonWidget.builder(Text.translatable("screen.universal_translator.log.clear"), button -> {
            NeoForgeTranslationRuntime.clearTranslationHistory();
            scroll = 0;
        ...
        addDrawableChild(ButtonWidget.builder(Text.translatable("screen.universal_translator.log.top_left"), button -> {
            NeoForgeConfig current = NeoForgeTranslationRuntime.currentConfig();
            if (current != null) {
    ...
    private void savePinned(boolean enabled, String preset, int x, int y, int width, int scale) {
        ...
        try {
            NeoForgeConfig updated = config.withPinnedLogSettings(enabled, preset, x, y, width, scale);
            updated.save();
            NeoForgeTranslationRuntime.updateConfig(updated);
    ...
    public boolean mouseDragged(Click click, double deltaX, double deltaY) {
        if (draggingScrollbar && click.button() == 0) {
            ...
            ScrollbarMetrics metrics = scrollbarMetrics(top, bottom, maxScroll);
            if (metrics != null) {
                scroll = scrollForMouseY(click.y(), metrics, top, maxScroll);
            }
            return true;
        }
    ...
    private interface StepAction {
        void apply(NeoForgeConfig config, int value);
    }
}
