...
final class StyledChatText {
    ...
    static String translationInput(StringVisitable source, Style fallbackStyle) {
        if (source == null) {
            return "";
        }
    ...
    static StringVisitable rebuild(
            StringVisitable source, String translatedTemplate, Style fallbackStyle) {
    ...
    private static Plan plan(StringVisitable source, Style fallbackStyle) {
        StyledText styled = styledText(source, fallbackStyle);
        boolean preserveOriginal =
                NeoForgeTranslationRuntime.translatedTextColor() == TranslationTextColor.ORIGINAL;
    ...
    private static StyledText styledText(StringVisitable source, Style fallbackStyle) {
        ...
        if (source instanceof Text) {
            ...
            parts = preferParts(parts,
                    componentParts((Text) source, fallbackStyle), expected);
            List<InlineTextureText.StyledPart> visualParts =
                    InlineTextureText.styledParts(((Text) source).asOrderedText());
    ...
    private static List<InlineTextureText.StyledPart> componentParts(
            Text source, Style fallbackStyle) {
        List<InlineTextureText.StyledPart> parts =
    ...
    private static void appendComponentParts(
        ...
        String ownText = source.copyContentOnly().getString();
        if (!ownText.isEmpty()) {
    ...
    private static Style configuredTranslationStyle() {
        ...
        switch (color) {
            case GREEN: return Style.EMPTY.withColor(Formatting.GREEN);
            case GOLD: return Style.EMPTY.withColor(Formatting.GOLD);
            case LIGHT_PURPLE: return Style.EMPTY.withColor(Formatting.LIGHT_PURPLE);
            case YELLOW: return Style.EMPTY.withColor(Formatting.YELLOW);
            case WHITE: return Style.EMPTY.withColor(Formatting.WHITE);
            case AQUA: return Style.EMPTY.withColor(Formatting.AQUA);
            case ORIGINAL:
