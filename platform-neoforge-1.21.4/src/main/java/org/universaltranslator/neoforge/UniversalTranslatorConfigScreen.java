...
final class UniversalTranslatorConfigScreen extends Screen implements LocalTranslationScreen {
    ...
    private final NeoForgeConfig original;
    ...
    UniversalTranslatorConfigScreen(Screen parent, NeoForgeConfig config) {
    ...
    protected void init() {
        ...
        this.cacheEditorButton = addDrawableChild(ButtonWidget.builder(
                Text.translatable("screen.universal_translator.option.cache_editor"), button -> {
                    if (this.client != null) {
        ...
        setButtonLabel(cacheEditorButton,
                Text.translatable("screen.universal_translator.option.cache_editor"));
        ...
        this.endpoint.setPlaceholder(Text.literal("http://127.0.0.1:5000/..."));
    ...
    private void refreshLabels() {
        setButtonLabel(enabledButton,
                Text.translatable("screen.universal_translator.option.automatic", onOff(enabled)));
        setButtonLabel(chatButton,
                Text.translatable("screen.universal_translator.option.chat", onOff(translateChat)));
        setButtonLabel(otherButton,
                Text.translatable("screen.universal_translator.option.other", onOff(translateOther)));
        setButtonLabel(cacheButton,
                Text.translatable("screen.universal_translator.option.cache", onOff(diskCache)));
        setButtonLabel(providerButton,
                Text.translatable("screen.universal_translator.option.change_provider"));
        setButtonLabel(displayButton, Text.translatable("screen.universal_translator.option.display",
        ...
        setButtonLabel(mixedTextButton,
                Text.translatable("screen.universal_translator.option.mixed", onOff(translateEnglishOnly)));
        setButtonLabel(colorButton,
                Text.translatable("screen.universal_translator.option.color", colorLabel(translatedTextColor)));
        setButtonLabel(downloadButton, isLlm()
                ? Text.translatable("screen.universal_translator.option.llm_settings")
                : (isOfficialProvider()
                ? Text.translatable("screen.universal_translator.option.official_settings", providerLabel())
                : (isTencent()
        ...
        setButtonLabel(modelButton,
                Text.translatable("screen.universal_translator.option.model", offlineModel.displayName()));
        setButtonLabel(outgoingButton,
                Text.translatable("screen.universal_translator.option.outgoing", onOff(translateOutgoing)));
        setButtonLabel(targetLanguageButton,
                Text.translatable("screen.universal_translator.option.target_preset",
    ...
    private void saveAndApply() {
        ...
        try {
            ...
            if (NeoForgeConfig.isOfficialProvider(selectedProvider)
            ...
            NeoForgeConfig updated = buildConfig();
            ...
            if (updated.enabled && NeoForgeConfig.isOfficialProvider(updated.provider)
            ...
            if (runtimeChanged) {
                try {
                    NeoForgeTranslationRuntime.initialize(original);
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        TranslatorUiText.drawLeft(context, this.textRenderer,
                Text.translatable("screen.universal_translator.endpoint_hint"),
                layout.right, layout.targetY - 11, 0xA0A0A0, layout.buttonWidth);
        String rawRuntimeStatus = NeoForgeTranslationRuntime.status();
        ...
        if (!status.isEmpty()) {
            ...
            TranslatorUiText.drawCentered(context, this.textRenderer, Text.literal(runtimeStatus),
        ...
        Text watermark = Text.literal("由承挨二次开发");
        context.drawTextWithShadow(this.textRenderer, watermark,
    ...
    private boolean isOfficialProvider() {
        return NeoForgeConfig.isOfficialProvider(provider);
    }
    ...
    private NeoForgeConfig buildConfig() {
        String selectedProvider = isLlm() ? "custom-api" : provider;
    ...
    private void saveClearedConfig() {
        NeoForgeConfig cleared = buildConfig();
        try {
            cleared.save();
        ...
        try {
            NeoForgeTranslationRuntime.initialize(cleared);
        } catch (Exception ignored) {
            // 下次保存刷新
        }
    }
    ...
    private static String tr(String key, Object... arguments) { return Text.translatable(key, arguments).getString(); }
