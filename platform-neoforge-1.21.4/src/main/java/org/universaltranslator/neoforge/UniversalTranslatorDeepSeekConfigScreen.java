...
final class UniversalTranslatorDeepSeekConfigScreen extends Screen implements LocalTranslationScreen {
    ...
    UniversalTranslatorDeepSeekConfigScreen(
        ...
        super(Text.translatable("screen.universal_translator.official.title", parent.providerLabel()));
        this.parent = parent;
    ...
    protected void init() {
        ...
        addDrawableChild(ButtonWidget.builder(Text.translatable("screen.universal_translator.deepseek.save"), button -> save())
                .dimensions(left, top + 72, buttonWidth, 20).build());
        addDrawableChild(ButtonWidget.builder(Text.translatable("gui.cancel"), button -> close())
                .dimensions(left + buttonWidth + gap, top + 72, buttonWidth, 20).build());
        addDrawableChild(ButtonWidget.builder(Text.translatable("screen.universal_translator.deepseek.clear"), button -> clearAll())
                .dimensions(left + (buttonWidth + gap) * 2, top + 72, buttonWidth, 20).build());
    }
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        TranslatorUiText.drawLeft(context, this.textRenderer,
                Text.translatable(providerKey("model")), left, top - 11, 0xFFFFFF, width);
        TranslatorUiText.drawLeft(context, this.textRenderer,
                Text.translatable(providerKey("api_key")), left, top + 25, 0xFFFFFF, width);
        if (!status.isEmpty()) {
